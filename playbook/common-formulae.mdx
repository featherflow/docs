...
<AccordionGroup>
<Accordion icon="" title="String Concatenation">
...
{{ @col1 }}.concat(" ", str2)
```
</Accordion> <Accordion icon="" title="String Splitting"> Split a string into an array using a separator.
```js
{{ @col1 }}.split(",") // Split by comma
{{ @col1 }}.split(",")[n] // nth split starting from 0
```
</Accordion> <Accordion icon="" title="String Trimming"> Trim whitespace from the start and end of a string.
```js
...
```
</Accordion> <Accordion icon="" title="Pad Start/End"> Add padding to the start or end of a string to reach a specified length.
```js
{{ @col1 }}.padStart(10, "0") // Pads with zeros at the start
{{ @col1 }}.padEnd(10, ".") // Pads with periods at the end
```
</Accordion> <Accordion icon="" title="Convert String to Array of Characters"> Convert a string into an array of characters.
```js
Array.from({{ @col1 }})
```
</Accordion> <Accordion icon="" title="Check if String is Empty"> Check if a string is empty (or just contains whitespace).
```js
{{ @col1 }}.trim() === ""
```
</Accordion>
