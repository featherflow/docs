...
5. [Endpoints](#5-endpoints)
   - [5.1 POST /grids/:gridId/run — Run a Grid](#51-run-a-grid)
   - [5.2 GET /run/status/:requestId — Get Run Status](#52-get-run-status)
   - [5.3 POST /api-key/rotate — Rotate API Key](#53-rotate-api-key)
...
4. To generate a new key, use [`POST /api/v1/api-key/rotate`](#53-rotate-api-key) or rotate it from the dashboard.
...
The BitScale External API enforces a default rate limit of **5 requests per second** per workspace.
